...
# --- Baking (ENABLED: use bake_all instead of REPLAY) ---
BAKE_CACHE_DIR  = "/home/jordan/Desktop/fire_bake"

# --- Visibility helper ---
FAST_VISIBILITY_MODE = True

# ====================== DRIVER FALLBACKS (AUTO) ======================
import re, time

_ALLOWED_DRIVER_NAMES = {
    # Blender's driver builtins / math you might legitimately use in expressions
    "frame", "pi", "e", "tau",  # frame is special-cased by Blender
    "sin", "cos", "tan", "asin", "acos", "atan",
    "sinh", "cosh", "tanh", "sqrt", "log", "log10", "pow", "abs", "min", "max", "floor", "ceil", "round",
    # Common module-like names sometimes used in drivers
    "math"
}

def ensure_driver_fallbacks_auto(default_value=1.0, verbose=True):
    """
    Scans all driver expressions and adds any unknown identifiers (like 'upres_sn3', 'sn3', 'gs_s3')
    into bpy.app.driver_namespace to prevent NameError during headless eval.
    """
    ns = bpy.app.driver_namespace
    ident_rx = re.compile(r"\b[A-Za-z_]\w*\b")

    def _maybe_register(expr, idname, datapath, idx):
        # Find all identifiers in expression
        idents = set(ident_rx.findall(expr or ""))
        for name in idents:
            # Skip numbers/allowed tokens/dunder-ish
            if name in _ALLOWED_DRIVER_NAMES:
                continue
            ...
            if name in {"True", "False", "None"}:
                continue
            # If Blender already knows it, skip
            if name in ns:
                continue
            # Heuristic: ignore obvious function calls (followed by '(') by checking raw occurrence
            if f"{name}(" in (expr or ""):
                continue
            ns[name] = float(default_value)
            if verbose:
                print(f"[drivers] set fallback: {name} = {ns[name]}  (in {idname} -> {datapath}[{idx}])")

    def _scan_id(idblock, idname):
        ...
        for fcu in ad.drivers:
            ...
            if not drv:
                continue
            expr = (drv.expression or "").strip()
            if expr:
                _maybe_register(expr, idname, getattr(fcu, "data_path", ""), getattr(fcu, "array_index", 0))

    for ob in bpy.data.objects:
        _scan_id(ob, f"Object('{ob.name}')")
    for m in bpy.data.materials:
        _scan_id(m, f"Material('{m.name}')")
    for w in bpy.data.worlds:
        _scan_id(w, f"World('{w.name}')")
    for s in bpy.data.scenes:
        _scan_id(s, f"Scene('{s.name}')")
# ====================================================================
...
def build_domain_and_flow():
    ...
    # --- Ensure Flow has no parent and lives in a separate collection ---
    flow.parent = None
    flow.matrix_parent_inverse.identity()

    # Put flow in a dedicated 'Sim' collection (keeps it separate from FX emitters)
    sim_col = bpy.data.collections.get("Sim") or bpy.data.collections.new("Sim")
    if sim_col.name not in bpy.context.scene.collection.children:
        bpy.context.scene.collection.children.link(sim_col)
    for col in flow.users_collection:
        col.objects.unlink(flow)
    sim_col.objects.link(flow)
    ...
    if not mat_flow:
        ...
        mat_flow.blend_method = 'BLEND'
        if hasattr(mat_flow, "shadow_method"):
            mat_flow.shadow_method = 'NONE'
        elif hasattr(mat_flow, "use_shadow"):
            mat_flow.use_shadow = False
...
def ensure_fire_flow(flow_obj):
    """Make the flow emit both fuel (for flame) and smoke, with enough heat."""
    fmod = flow_obj.modifiers.get("Fluid")
    if not fmod or fmod.type != 'FLUID' or fmod.flow_settings is None:
        print("[flow] No flow_settings on", flow_obj.name); return
    fs = fmod.flow_settings

    # Emit fire + smoke (not just smoke)
    # Valid: 'SMOKE', 'FIRE', 'BOTH'
    fs.flow_type = 'BOTH'

    # Inflow vs geometry: keep what you had; here's a safe default
    # Valid: 'INFLOW', 'OUTFLOW', 'GEOMETRY'
    if not hasattr(fs, "flow_behavior") or fs.flow_behavior not in {'INFLOW','OUTFLOW','GEOMETRY'}:
        fs.flow_behavior = 'GEOMETRY'

    # Emit enough fuel and heat so the sim actually ignites
    if hasattr(fs, "fuel_amount"):
        fs.fuel_amount = 3.0   # try 1.0–3.0
    if hasattr(fs, "temperature"):
        fs.temperature  = 2.0  # try 0.5–2.0 (relative heat)
    elif hasattr(fs, "temperature_difference"):
        fs.temperature_difference = 1.5

    print("[flow] Flow set to BOTH with fuel & temperature:", flow_obj.name)

def apply_fire_volume_material(domain_obj, mat_name="FireVolume"):
    """Create/assign a Principled Volume shader that shows flame + smoke."""
    import bpy
    # Make/get material
    ...
    pv  = nt.nodes.new("ShaderNodeVolumePrincipled"); pv.location = (350, 0)

    vinf = nt.nodes.new("ShaderNodeVolumeInfo"); vinf.location = (0, 0)                  # gives Density, Flame, Temperature
    bblk = nt.nodes.new("ShaderNodeBlackbody");  bblk.location = (180, -180)             # temperature -> color

    # Strength scale for flame (flame is 0..1; scale up to be bright)
    m_flame = nt.nodes.new("ShaderNodeMath"); m_flame.operation = 'MULTIPLY'
    m_flame.location = (180, -20); m_flame.inputs[1].default_value = 12.0                # tweak 8–25

    # Map temperature (often 0..1) into realistic K (e.g., 1300K–2200K)
    mapT = nt.nodes.new("ShaderNodeMapRange"); mapT.location = (0, -180)
    mapT.inputs[1].default_value = 0.0   # From Min
    mapT.inputs[2].default_value = 1.0   # From Max
    mapT.inputs[3].default_value = 1300  # To Min (Kelvin)
    mapT.inputs[4].default_value = 2200  # To Max (Kelvin)
    mapT.clamp = True

    # Optional: smoke density scale
    m_dens = nt.nodes.new("ShaderNodeMath"); m_dens.operation = 'MULTIPLY'
    m_dens.location = (180, 130); m_dens.inputs[1].default_value = 1.2                   # tweak 0.5–3

    # Links
    ln = nt.links
    # Smoke density
    ln.new(vinf.outputs["Density"], m_dens.inputs[0])
    ln.new(m_dens.outputs[0], pv.inputs["Density"])
    # Flame emission strength
    ln.new(vinf.outputs["Flame"], m_flame.inputs[0])
    ln.new(m_flame.outputs[0], pv.inputs["Emission Strength"])
    # Blackbody color from mapped temperature
    ln.new(vinf.outputs["Temperature"], mapT.inputs["Value"])
    ln.new(mapT.outputs["Result"],      bblk.inputs["Temperature"])
    ln.new(bblk.outputs["Color"],       pv.inputs["Emission Color"])
    # Output
    ln.new(pv.outputs["Volume"], out.inputs["Volume"])

    # Assign to the domain object
    ...
    print(f"[mat] Applied volume flame material '{mat_name}' to domain:", domain_obj.name)
...
def make_fire_material(domain, ctrl):
    ...
    gain.inputs[0].default_value = 350.0
    ...
    if FAST_VISIBILITY_MODE:
        try:
            vol.inputs["Emission Strength"].default_value = max(260.0, vol.inputs["Emission Strength"].default_value)
...
def build_sparks_and_ripples(ceiling_obj, spark_obj): # Added spark_obj parameter
    ...
    emitter.location.z = FLOOR_Z + 0.95   # instead of 0.6

    # Keep FX separate from Sim so nothing gets parented accidentally
    fx_col = bpy.data.collections.get("FX") or bpy.data.collections.new("FX")
    if fx_col.name not in bpy.context.scene.collection.children:
        bpy.context.scene.collection.children.link(fx_col)
    for col in emitter.users_collection:
        col.objects.unlink(emitter)
    fx_col.objects.link(emitter)
    ...
    # Don't render the emitter mesh
    # Cross-version way to hide the emitter while keeping the particles visible
    ps = emitter.particle_systems[0]
    particle  = ps.settings

    # Blender ≤3.x had this:
    if hasattr(particle, "use_render_emitter"):
        particle.use_render_emitter = False

    # Blender 3.x/4.x compatible: hide the instancer (emitter) at render time,
    # but still render the particles themselves.
    if hasattr(emitter, "show_instancer_for_render"):
        emitter.show_instancer_for_render = False

    # (Optional belt-and-suspenders for Cycles; safe in most cases)
    if hasattr(emitter, "cycles_visibility"):
        # Leave diffuse/glossy visibility alone; only hide the emitter mesh from camera
...
def main():
    ...
    ensure_fire_flow(flow)

    apply_fire_volume_material(domain)
    ...
    ensure_driver_fallbacks_auto(default_value=1.0)


    # ====================== BAKE / RENDER (TWO-PHASE, MODULAR) ======================
    import sys
    import argparse

    def _get_phase():
        # 1) env var (easiest in shell): PHASE=BAKE or PHASE=RENDER
        phase = os.environ.get("PHASE", "").strip().upper()
        if phase in {"BAKE", "RENDER"}:
            return phase
        # 2) optional CLI arg: --phase=BAKE|RENDER
        try:
            parser = argparse.ArgumentParser(add_help=False)
            parser.add_argument("--phase", choices=["BAKE", "RENDER"], default="RENDER")
    ...
    # Always have a scene handle here
    scn = bpy.context.scene

    # In ensure_domain_cache(domain_obj), switch to a per-run subdir:
    def ensure_domain_cache(domain_obj):
        try:
            dm = domain_obj.modifiers.get("Fluid")
            d = dm.domain_settings if dm else None
        except Exception:
            d = None
        if d is None:
            print("WARNING: No Fluid domain_settings found on 'domain'.")
            return

        d.cache_type = 'MODULAR'
        if hasattr(d, "cache_frame_start"):
            d.cache_frame_start = bpy.context.scene.frame_start
        if hasattr(d, "cache_frame_end"):
            d.cache_frame_end = bpy.context.scene.frame_end

        # Per-run cache dir to avoid "grids are still in use" after a crash
        root_dir = bpy.path.abspath(BAKE_CACHE_DIR) if 'BAKE_CACHE_DIR' in globals() else "/tmp/blender_fluid_cache"
        run_dir = os.path.join(root_dir, f"run_{int(time.time())}")
        os.makedirs(run_dir, exist_ok=True)
        if hasattr(d, "cache_directory"):
            d.cache_directory = run_dir

        print(f"[cache] type=MODULAR range={bpy.context.scene.frame_start}-{bpy.context.scene.frame_end} dir={run_dir}")

    # ---------- DEFERRED BAKE HELPERS (no depsgraph update during eval) ----------
    def _select_active_only(obj):
        for o in bpy.context.selected_objects:
            o.select_set(False)
        obj.select_set(True)
        bpy.context.view_layer.objects.active = obj

    def start_deferred_bake_and_quit(domain_obj, after_save_quit=True):
        """Defer bake_all() to avoid eval-state crashes, then wait until finished."""
        # Ensure cache & selection are set up now (no updates/flushes here)
        ensure_domain_cache(domain_obj)
        _select_active_only(domain_obj)

        # 1) Defer the actual bake to next tick
        def _kick_bake():
            # Cancel playback just in case; do not call view_layer.update() here
            try:
                bpy.ops.screen.animation_cancel(restore_frame=False)
            except Exception:
                pass
            # Go to a stable frame (safe; no flush)
            bpy.context.scene.frame_set(bpy.context.scene.frame_start)
            # Free then bake; if the operator still complains, re-queue this timer
            try:
            ...
            try:
                ...
                print("[bake] retrying deferred bake:", e)
                return 0.3  # try again shortly
            return None  # started bake successfully

        bpy.app.timers.register(_kick_bake, first_interval=0.1)

        # 2) Poll until the domain is no longer baking, then save & quit
        def _watch_bake():
            try:
                d = domain_obj.modifiers["Fluid"].domain_settings
            except Exception:
                # If domain/modifier vanished, just exit to avoid spinning
                print("[bake] domain missing; exiting watcher")
                if after_save_quit:
                    bpy.ops.wm.quit_blender()
                return None

            # Check Mantaflow baking flags (present in 3.x/4.x)
            if any(getattr(d, k, False) for k in ("is_baking", "is_baking_data", "is_baking_noise")):
                return 0.5  # still baking; poll again

            # Done baking -> finalize
            print("[bake] finished. Saving/Exiting…")
            # Keep camera constraint if needed (no depsgraph flush)
            if 'CEILING_Z' in globals():
                try:
        ...
        bpy.app.timers.register(_watch_bake, first_interval=1.0)
    # ---------------------------------------------------------------------------

    if PHASE == "BAKE":
        print("[phase] BAKE")
        # Make sure any driver fallbacks are set before baking (from earlier step)
        try:
            ensure_driver_fallbacks_auto(default_value=1.0)
        except Exception:
            pass

        # Kick off deferred bake; this process will remain alive until watchers quit
        start_deferred_bake_and_quit(domain, after_save_quit=True)

        # IMPORTANT: do not quit here; timers will handle save & exit after the bake
    else:
        print("[phase] RENDER")
        # Fresh process; assume cache baked
        # No flushing—just render
        scn = bpy.context.scene
